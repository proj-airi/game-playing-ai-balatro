...
class AutoClickService:
    ...
    def __init__(self, screen_capture: ScreenCapture):
        ...
        logger.info(
            f'Auto-click service initialized with {self.cooldown_seconds}s cooldown'
        )
    ...
    def find_first_card(self, detections: List[Detection]) -> Optional[Detection]:
        ...
        for i, det in enumerate(detections):
            ...
            # Fallback to general card keywords but with lower priority
                ...
                if (
                    'description' not in class_name_lower
                    and 'back' not in class_name_lower
                ):
                    is_playable_card = True
                    card_type = 'generic_card'

            logger.info(
                f'  {i + 1}. {det.class_name} -> {card_type} ({"✓" if is_playable_card else "✗"})'
            )
            logger.info(
                f'      Position: {det.bbox}, Center: {det.center}, Confidence: {det.confidence:.3f}'
            )
        ...
        for i, (det, card_type) in enumerate(card_detections):
            ...
            logger.info(
                f'  {i + 1}. {det.class_name} ({card_type}) at x={det.bbox[0]} {marker}'
            )

        selected_card = card_detections[0][0]
        logger.info(
            f'✅ Selected first card: {selected_card.class_name} at {selected_card.center}'
        )
    ...
    def click_first_card(self, detections: List[Detection]) -> bool:
        ...
        if time_since_last < self.cooldown_seconds:
            logger.info(
                f'⏰ Click on cooldown: {time_since_last:.1f}s < {self.cooldown_seconds}s'
            )
            return False
        ...
        if (
            region_x < 0
            or region_y < 0
            or region_x > region_width
            or region_y > region_height
        ):
        ...
        try:
            ...
            logger.info(
                f'   🖱️ Final screen coordinates: ({screen_x:.1f}, {screen_y:.1f})'
            )
    ...
    def print_click_status(self) -> None:
        ...
        logger.info(
            f'   🕐 Time since last click: {status["time_since_last_click"]:.1f}s'
        )
    ...
    def _check_macos_permissions(self) -> None:
        ...
        try:
            ...
            current_pos = self.mouse_controller.position
            logger.info(
                f'✅ macOS accessibility permissions OK - current mouse position: {current_pos}'
            )
        except Exception as e:
            logger.error('❌ macOS accessibility permissions may be missing!')
            logger.error(
                '   Please enable accessibility permissions for this application:'
            )
            logger.error(
    ...
    def test_click_permissions(self) -> bool:
        ...
        try:
            ...
            current_pos = self.mouse_controller.position
            logger.info(
                f'📍 Testing click permissions - current mouse at: {current_pos}'
            )

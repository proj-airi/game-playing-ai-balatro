...
class DetectionService:
    ...
    def __init__(self, model_path: Optional[str] = None, use_onnx: bool = False, 
        ...
        self.auto_click_service = AutoClickService(self.screen_capture) if enable_auto_click else None
    ...
    def run_single_detection(self, save_result: bool = True) -> bool:
        ...
        for i, det in enumerate(detections):
            logger.info(f"  {i+1}. {det.class_name} (confidence: {det.confidence:.3f}) position: {det.bbox}")
        ...
        window_name = settings.ui_window_names.get('single_detection', 'Balatro Detection Result')
    ...
    def run_continuous_detection(self, fps: int = 2) -> None:
        ...
        while True:
            if not paused:
                ...
                # Note: Continuous mode doesn't auto-click, only manual trigger with 'c' key
                ...
                info_text = [
                    ...
                    "Space:Pause q:Exit s:Save +/-:Adjust confidence" + (" c:Click" if self.auto_click_service else "")
                ]
                ...
                # Display results (position window to avoid game window overlap)
                window_name = settings.ui_window_names.get('continuous_detection', 'Balatro Real-time Detection')
            ...
            if key == ord('q'):
                ...
                if not paused:
                    ...
                    if frame is not None:
                        ...
                        if detections:
                            clicked = self.auto_click_service.manual_click_first_card(detections)
                            if clicked:
                    ...
                    logger.info("Please resume detection first (press Space)")
        ...
        logger.info(f"  Average detections/frame: {self.detection_count / self.frame_count:.1f}")
    ...
    def _position_display_window(self, window_name: str, window_type: str) -> None:
        ...
        logger.info(f"Display window position: ({display_x}, {display_y}), "

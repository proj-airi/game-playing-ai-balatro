...
class BalatroDetectionDemo:
    ...
    def setup(self) -> bool:
        ...
        auto_click_input = input("Enable auto-click on first card? (y/N): ").strip().lower()
        ...
        if auto_click:
            ...
            cooldown_input = input("Click cooldown time (seconds, default 1.0): ").strip()
            if cooldown_input:
                try:
        ...
        if confidence:
            try:
                self.detection_service.set_detection_params(confidence=float(confidence))
            except ValueError:
                logger.warning("Invalid input, using default value")

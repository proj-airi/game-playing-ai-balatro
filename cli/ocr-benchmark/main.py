...
def build_variants(image):
    ...
    for name, img in variants.items():
        for sf in (1, 2, 3):
            if sf == 1:
                ...
                scaled = cv2.resize(
                    img, None, fx=sf, fy=sf, interpolation=cv2.INTER_CUBIC
                )
    ...
    if 'grayscale_2x' in variants_scaled:
        denoised = cv2.bilateralFilter(
            variants_scaled['grayscale_2x'], d=5, sigmaColor=60, sigmaSpace=60
        )
...
def run(
    ...
    with TestOutputManager(out_name, keep_outputs=True) as mgr:
        ...
        for img_path in paths:
            ...
            for idx, d in enumerate(descs):
                ...
                for vname, vimg in variants.items():
                    ...
                    for eng in engines:
                        logger.info(
                            f'OCR {img_path.name} desc#{idx} variant={vname} with {eng.name}'
                        )
                ...
                if judge is not None:
                    judge_res = judge.score(
                        candidate_texts,
                        context=f'Balatro card description from {img_path.name}',
                    )
                    (img_out / f'desc_{idx:02d}_judge.json').write_text(
                        json.dumps(judge_res, indent=2, ensure_ascii=False)
                    )
        ...
        (out_dir / 'descriptions_details.json').write_text(
...
def main() -> int:
    ...
    p.add_argument(
        '--targets', nargs='*', help='Specific filenames (e.g., out_00104.jpg)'
    )
    p.add_argument(
        '--lang', default='en', help="OCR languages, e.g., 'en' or 'en,ch_sim'"
    )
    p.add_argument('--judge-model', help='OpenRouter model id for LLM judge (optional)')
    p.add_argument('--out', default='ocr_benchmarks', help='Output folder name')
    args = p.parse_args()

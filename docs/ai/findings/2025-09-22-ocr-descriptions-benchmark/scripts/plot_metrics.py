...
def agg(rows: List[Dict[str, Any]]):
    ...
    for eng, d in stats.items():
        ...
        d['avg_text_len_success'] = (
            (d['sum_text'] / d['success']) if d['success'] else 0
        )
...
def plot(stats: Dict[str, Dict[str, Any]]):
    ...
    bar(ax, engines, sr, 'Success Rate by Engine', 'Percent')
    fig.tight_layout()
    p = ASSETS / 'success_rate.png'
    fig.savefig(p)
    plt.close(fig)
    out['success_rate'] = p.name

    fig, ax = plt.subplots(figsize=(6, 4))
    bar(ax, engines, t, 'Avg Time (successful) by Engine', 'Seconds')
    fig.tight_layout()
    p = ASSETS / 'avg_time_success.png'
    fig.savefig(p)
    plt.close(fig)
    out['avg_time_success'] = p.name

    fig, ax = plt.subplots(figsize=(6, 4))
    bar(ax, engines, txt, 'Avg Extracted Text Length (successful)', 'Characters')
    fig.tight_layout()
    p = ASSETS / 'avg_text_len_success.png'
    fig.savefig(p)
    plt.close(fig)
    out['avg_text_len_success'] = p.name

    fig, ax = plt.subplots(figsize=(6, 4))
    bar(ax, engines, eff, 'Chars per Second (successful)', 'chars/sec')
    fig.tight_layout()
    p = ASSETS / 'efficiency.png'
    fig.savefig(p)
    plt.close(fig)
    out['efficiency'] = p.name
...
def write_readme(stats: Dict[str, Dict[str, Any]], assets: Dict[str, str]):
    ...
    best_success = (
        max(stats.items(), key=lambda kv: kv[1]['success_rate'])[0] if stats else 'N/A'
    )
    best_eff = (
        max(stats.items(), key=lambda kv: kv[1]['avg_efficiency'])[0]
        if stats
        else 'N/A'
    )
    frontmatter = {

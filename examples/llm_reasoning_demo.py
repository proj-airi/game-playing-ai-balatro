...
class LLMReasoningDemo:
    ...
    def initialize_components(self) -> bool:
        ...
        try:
            ...
            if self.llm_provider.initialize():
                print(f"   ✅ LLM Provider initialized: {self.llm_provider.config.model_name}")
            else:
            ...
            for model_name, info in model_info.items():
                if info['available']:
                    print(f"     - {model_name.upper()}: {info['classes_count']} classes")
    ...
    def _demo_single_turn(self):
        ...
        try:
            ...
            if analysis_result.success:
                ...
                print(f"   Game State: {len(analysis_result.metadata.get('game_state', {}).get('cards', []))} cards detected")
                print(f"   Entities: {analysis_result.metadata.get('entities_count', 0)} entities")
                print(f"   UI Elements: {analysis_result.metadata.get('ui_elements_count', 0)} elements")
                print(f"\n🤔 AI Analysis:")
            ...
            if planning_result.success:
                ...
                if planning_result.action:
                    print(f"   Planned Action: {planning_result.action.get('name', 'unknown')}")
                    print(f"   Arguments: {planning_result.action.get('arguments', {})}")
                else:
                    print(f"   No specific action planned")
            ...
            if planning_result.action:
                ...
                if execute in ['y', 'yes']:
                    ...
                    print(f"⏸️  Action execution skipped")
    ...
    def _demo_multi_turn(self):
        ...
        try:
            ...
            for turn in range(1, num_turns + 1):
                ...
                print(f"🤖 Running AI reasoning cycle...")
                ...
                if result.success:
                    ...
                    if result.action:
                        print(f"   Action taken: {result.action.get('name', 'unknown')}")
            ...
            if history:
                print(f"\n📈 Action History:")
                for i, action in enumerate(history[-3:], max(1, len(history)-2)):  # Show last 3
                    status = "✅" if action['success'] else "❌"
                    print(f"   {i}. {status} {action['action'].get('name', 'unknown')} "
    ...
    def _demo_interactive(self):
        ...
        try:
            ...
            while True:
                try:
                    question = input("\n❓ Ask about strategy, cards, or actions: ").strip()
...
Provide a helpful, strategic answer based on Balatro game mechanics."""
    ...
    def _demo_history(self):
        ...
        try:
            ...
            position_actions = [a for a in history if 'positions' in a['action'].get('arguments', {})]
            if position_actions:
                avg_play_count = sum(

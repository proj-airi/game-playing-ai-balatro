...
def create_huggingface_dataset(
    ...
    print(f"- Image paths in metadata reference 'yolo/images/' directory")
    print(f"\nDataset structure:")
    print(f"{output_path}/")
    print(f"└── data/")
    print(f"    └── {split_name}/")
...
def main():
    ...
    parser.add_argument(
        "--split", 
        default="train",
        help="Split name (e.g., train, test, val)"
    )
    ...
    missing_items = [item for item in required_items if not (source_path / item).exists()]
    if missing_items:
        print(f"Error: Missing required directories in source: {missing_items}")
        return 1
    
    print(f"Converting YOLO dataset:")

...
def plot_figures(stats: Dict[str, Dict[str, Any]], out_dir: Path) -> Dict[str, str]:
    ...
    fig, ax = plt.subplots(figsize=(6, 4))
    bar(ax, engines, avg_text_len, "Avg Extracted Text Length (successful)", "Characters")
...
def write_doc(doc_dir: Path, title: str, stats: Dict[str, Dict[str, Any]], asset_files: Dict[str, str]):
...
def main() -> int:
    ...
    p.add_argument("--input-dir", default="apps/tests/outputs/ocr_descriptions", help="Benchmark output dir")
    p.add_argument("--doc-name", default="2025-09-22-ocr-descriptions-benchmark", help="Folder name under docs/ai/findings/")
    p.add_argument("--title", default="Balatro OCR (Descriptions) Benchmark Findings", help="Doc title")
    args = p.parse_args()

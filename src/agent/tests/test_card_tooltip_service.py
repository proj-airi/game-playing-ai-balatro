...
class TestCardTooltipService:
    ...
    def mock_detections(self):
        ...
        detections = [
            ...
            Detection(
                3, 'poker_card_description', 0.82, (520, 100, 650, 180)
            ),  # Near joker
        ]
        return detections
    ...
    def test_match_tooltips_to_cards_with_stats(
        self, tooltip_service, mock_detections, test_output_dir
    ):
        """Test tooltip-card matching with detailed statistics."""
        matches = tooltip_service.match_tooltips_to_cards(
            mock_detections, max_distance=200
        )
    ...
    def test_create_tooltip_card_crops_with_saving(
        ...
        enhanced_matches = tooltip_service.create_tooltip_card_crops(
            mock_balatro_image, matches
        )
        ...
        for i, match in enumerate(enhanced_matches):
            ...
            assert match['tooltip_crop'].shape[2] == 3, (
                'Tooltip crop should be color image'
            )
            assert match['card_crop'].shape[2] == 3, 'Card crop should be color image'
            assert match['context_crop'].shape[2] == 3, (
                'Context crop should be color image'
            )
    ...
    def test_create_visualization_with_annotations(
        ...
        for i, match in enumerate(matches):
            # Get centers
            tooltip_center = tooltip_service.region_matcher.get_bbox_center(
                match['tooltip_bbox']
            )
            card_center = tooltip_service.region_matcher.get_bbox_center(
                match['card_bbox']
            )
            ...
            cv2.putText(
                vis_image,
                f'Match {i}: {match["distance"]:.1f}px',
                (mid_x, mid_y),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                match_line_color,
        ...
        cv2.imwrite(
            str(test_output_dir / 'complete_matching_visualization.png'), vis_image
        )
        ...
        cv2.imwrite(
            str(test_output_dir / 'final_visualization_with_legend.png'), final_image
        )

        print('✓ Comprehensive visualization saved')

    def test_pixel_level_validation(
        self, tooltip_service, mock_balatro_image, mock_detections, test_output_dir
    ):
        ...
        for i, match in enumerate(enhanced_matches):
            ...
            assert tooltip_stats['mean_brightness'] > 0, (
                'Tooltip should have some brightness'
            )
            assert card_stats['mean_brightness'] > 0, 'Card should have some brightness'
            assert tooltip_stats['pixel_count'] > 100, (
                'Tooltip should have reasonable pixel count'
            )
            assert card_stats['pixel_count'] > 100, (
                'Card should have reasonable pixel count'
            )
        ...
        with open(pixel_stats_file, 'w') as f:
            ...
            for stats in pixel_stats:
                ...
                f.write(
                    f'  Tooltip - Brightness: {stats["tooltip"]["mean_brightness"]:.1f} ± {stats["tooltip"]["std_brightness"]:.1f}\n'
                )
                f.write(
                    f'  Tooltip - Dominant Color (BGR): {stats["tooltip"]["dominant_color"]}\n'
                )
                f.write(f'  Tooltip - Pixel Count: {stats["tooltip"]["pixel_count"]}\n')
                f.write(
                    f'  Card - Brightness: {stats["card"]["mean_brightness"]:.1f} ± {stats["card"]["std_brightness"]:.1f}\n'
                )
                f.write(
                    f'  Card - Dominant Color (BGR): {stats["card"]["dominant_color"]}\n'
                )

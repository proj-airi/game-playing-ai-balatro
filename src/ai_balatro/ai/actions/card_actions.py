...
class CardPositionDetector:
    ...
    def get_hand_cards(self, detections: List[Detection]) -> List[Detection]:
        ...
        for i, card in enumerate(hand_cards):
            logger.info(
                f'  位置{i}: {card.class_name} at {card.center} (置信度: {card.confidence:.3f})'
            )
...
class DetectionVisualizer:
    ...
    def show_detection_results(
        ...
        try:
            ...
            colors = {
                'card': (0, 255, 0),  # 绿色 - 牌类
                'button': (255, 0, 0),  # 红色 - 按钮
                'ui': (0, 255, 255),  # 黄色 - UI元素
                'other': (128, 128, 128),  # 灰色 - 其他
            }

            logger.info(f'显示{len(detections)}个检测结果')
            ...
            # 添加统计信息
            stats_text = [
                f'总检测数: {len(detections)}',
                f'牌类: {self._count_detections_by_type(detections, "card")}',
                f'按钮: {self._count_detections_by_type(detections, "button")}',
                f'其他: {len(detections) - self._count_detections_by_type(detections, "card") - self._count_detections_by_type(detections, "button")}',
                '按 ESC 关闭窗口',
            ]
            ...
            logger.info(f'显示检测结果窗口: {window_title}')
            logger.info('按ESC键关闭窗口...')
            ...
            if key == 27:  # ESC键
                logger.info('用户关闭检测结果窗口')

        except Exception as e:
            logger.error(f'显示检测结果时发生错误: {e}')
...
class ButtonDetector:
    ...
    def __init__(self, multi_detector: Optional[MultiYOLODetector] = None):
        ...
        Args:
            multi_detector: 多模型检测器实例，如果为None则创建新的
        """
        self.multi_detector = multi_detector

        # 按钮类型映射表，从UI模型检测类名到标准按钮类型
    ...
    def find_buttons(
        ...
        Args:
            image: 输入图像
            target_type: 目标按钮类型，如果为None则返回所有按钮
            show_visualization: 是否显示检测结果可视化窗口
            confidence_threshold: 置信度阈值
            iou_threshold: IoU阈值

        Returns:
            按钮ButtonDetection列表，按置信度排序
        """
        ...
        try:
            ...
            if buttons:
                logger.info(f'UI模型检测到{len(buttons)}个按钮:')
                for i, button in enumerate(buttons):
                    logger.info(
                        f'  按钮{i}: {button.button_type} ({button.class_name}) at {button.center} (置信度: {button.confidence:.3f})'
                    )
            else:
                logger.warning(
                    f'UI模型未检测到{"任何" if target_type is None else target_type}按钮'
                )
                logger.info(f'UI模型总共检测到{len(ui_detections)}个对象:')
                for i, det in enumerate(ui_detections[:5]):  # 显示前5个
                    logger.info(
                        f'  {i + 1}. {det.class_name} (置信度: {det.confidence:.3f})'
                    )
                if len(ui_detections) > 5:
                    logger.info(f'  ... 还有{len(ui_detections) - 5}个对象')

            return buttons

        except Exception as e:
            logger.error(f'UI检测失败: {e}')
            return []
    ...
    def find_best_button(
        ...
        Returns:
            最佳按钮ButtonDetection，如果未找到返回None
        """
    ...
    def _get_button_type(self, detection: Detection) -> str:
        ...
        # 默认返回通用按钮类型
        logger.warning(f'无法识别按钮类型: {class_name}，返回默认类型')
        return 'unknown'
...
class CardActionEngine:
    ...
    def __init__(
        ...
        # 支持向后兼容的单一检测器或新的多模型检测器
        if multi_detector is not None:
            self.multi_detector = multi_detector
            self.yolo_detector = None  # 使用多模型时不需要单一检测器
            logger.info('使用多模型YOLO检测器')
        elif yolo_detector is not None:
            self.yolo_detector = yolo_detector
            self.multi_detector = None
            logger.info('使用传统单一YOLO检测器（向后兼容模式）')
        else:
            # 默认创建多模型检测器
            self.multi_detector = MultiYOLODetector()
            self.yolo_detector = None
            logger.info('创建默认多模型YOLO检测器')
        ...
        logger.info(
            f'鼠标移动设置: 持续时间={self.mouse_move_duration}s, 步数={self.mouse_move_steps}, 点击保持={self.click_hold_duration}s'
        )
        logger.info(
            f'窗口焦点设置: 启用={self.ensure_window_focus}, 方法={self.focus_method}'
        )
    ...
    def set_mouse_animation_params(
        ...
        logger.info(
            f'更新鼠标移动设置: 持续时间={move_duration}s, 步数={move_steps}, 点击保持={click_hold_duration}s'
        )
    ...
    def execute_card_action(
        ...
        try:
            ...
            if self.multi_detector is not None:
                ...
                logger.info(f'多模型检测器(实体模型)检测到{len(detections)}个对象')
            else:
                # 向后兼容：使用单一检测器
                detections = self.yolo_detector.detect(frame)
                logger.info(f'单一检测器检测到{len(detections)}个对象')
            ...
            if len(positions) > len(hand_cards):
                logger.warning(
                    f'位置数组长度({len(positions)})超过手牌数量({len(hand_cards)})'
                )
                # 截断位置数组
    ...
    def _execute_confirm_action(
        ...
        try:
            ...
            # 使用UI模型检测按钮
            logger.info(f'使用UI模型检测{button_type}按钮...')

            # 直接使用button_detector检测按钮
            target_button = self.button_detector.find_best_button(frame, button_type)

            if target_button is None:
                logger.warning(f'未找到{button_type}按钮，尝试寻找所有按钮...')
                all_buttons = self.button_detector.find_buttons(frame)

                if all_buttons:
                    logger.info(
                        f'找到{len(all_buttons)}个可用按钮，但没有{button_type}按钮'
                    )

                    # 显示所有按钮的可视化（如果启用）
                    if show_visualization:
                        logger.info('显示所有检测到的按钮...')
                        # 创建一个仅包含按钮的检测列表用于可视化
                    ...
                    # 可以考虑使用其他合适的按钮作为备选
                    logger.info('请手动点击相应按钮完成操作')
                else:
                    logger.error('未检测到任何按钮')

                    # 显示UI检测结果用于调试
                    if show_visualization and self.multi_detector is not None:
                        logger.info('显示UI模型检测结果...')
            ...
            # 点击找到的按钮
            logger.info(f'找到{button_type}按钮，准备点击...')
            success = self._click_button(target_button)

            if success:
                logger.info(f'✓ 成功点击{button_type}按钮')
            else:
                logger.error(f'点击{button_type}按钮失败')
    ...
    def _click_button(self, button: ButtonDetection) -> bool:
        ...
        try:
            ...
            logger.info(f'点击{button.button_type}按钮: {button.class_name}')
            logger.info(f'  检测中心: ({center_x}, {center_y})')
            logger.info(f'  屏幕坐标: ({screen_x:.1f}, {screen_y:.1f})')
            logger.info(f'  置信度: {button.confidence:.3f}')

            # 平滑移动鼠标到按钮位置
            if not self._smooth_move_mouse(int(screen_x), int(screen_y)):
                logger.error('平滑移动鼠标到按钮失败')
                return False
    ...
    def _smooth_move_mouse(self, target_x: int, target_y: int) -> bool:
        ...
        try:
            ...
            logger.info(
                f'平滑移动鼠标: ({start_x}, {start_y}) -> ({target_x}, {target_y})'
            )

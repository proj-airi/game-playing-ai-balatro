...
from .schemas import ActionType, CardAction, GAME_ACTIONS, BUTTON_CONFIG
...
class ActionExecutor(BaseProcessor):
    ...
    def __init__(self, 
        ...
        # 初始化子执行器
        self.card_engine = CardActionEngine(yolo_detector, screen_capture)
    ...
    def process(self, input_data: Any, context: Optional[Dict] = None) -> ProcessingResult:
        """
        处理动作请求。
    ...
    def _execute_function_call(self, function_call: Dict[str, Any]) -> ProcessingResult:
        ...
        if function_name == "select_cards_by_position":
            ...
            return ProcessingResult(
                success=False,
                data=None,
                errors=[f"未知的函数: {function_name}"]
            )
    
    def _execute_card_positions(self, args: Dict[str, Any], show_visualization: bool = False) -> ProcessingResult:
        """执行基于位置数组的牌操作。"""
        ...
        if not all(val in [-1, 0, 1] for val in positions):
            return ProcessingResult(
                success=False,
                data=None,
                errors=["位置数组只能包含-1, 0, 1"]
            )
        
        success = self.card_engine.execute_card_action(positions, description, show_visualization)
    ...
    def execute_from_array(self, positions: List[int], description: str = "", show_visualization: bool = False) -> bool:
        """
        便捷方法：直接从位置数组执行动作。
    ...
    def set_mouse_animation_params(self, 
                                   ...
                                   move_steps: int = 15, 
                                   click_hold_duration: float = 0.08) -> None:
        """
        设置鼠标移动动画参数。
    ...
    def set_window_focus_settings(self, 
                                  enable: bool = True, 

...
from src.ai_balatro.ai.templates import template_manager, render_prompt
from src.ai_balatro.ai.agents import BaseAgent, AgentContext, AgentResult, AgentOrchestrator
...
def mock_provider():
    ...
    return MockLLMProvider(
        ...
        function_calls=[
            {"name": "select_card", "arguments": {"index": 0}}
        ]
    )
...
class TestConversationMemory:
    ...
    def test_message_management(self):
        ...
        non_system_messages = [m for m in conversation.messages if m.role != MessageRole.SYSTEM]
...
class TestRealProviderIntegration:
    ...
    def test_openrouter_provider_integration(self):
        ...
        with provider:
            result = provider.generate_text("Hello, respond with 'Integration test successful'")

...
class TestImageCropper:
    ...
    def test_crop_multiple_regions_with_validation(
        self, cropper, test_image, test_output_dir
    ):
        ...
        for i, (crop, color, expected_pixel) in enumerate(
            zip(crops, colors, expected_bgr)
        ):
            # Save each crop
            ...
            assert center_pixel[dominant_channel] > 200, (
                f'{color} crop should have high {dominant_channel} channel'
            )

            print(f'✓ {color} crop saved: {crop.shape}, center pixel: {center_pixel}')

    def test_create_combined_crop_with_stats(
        self, cropper, test_image, test_output_dir
    ):
        ...
        print(
            f'✓ Combined crop: {combined.shape}, red pixels: {red_pixels}, green pixels: {green_pixels}'
        )
    ...
    def test_edge_case_cropping(self, cropper, test_image, test_output_dir):
        ...
        for case_name, bbox in edge_cases:
            ...
            assert cropped.shape[0] > 0, (
                f'{case_name} crop should not be empty (height)'
            )
            assert cropped.shape[1] > 0, f'{case_name} crop should not be empty (width)'
            assert cropped.shape[0] <= 90, (
                f'{case_name} crop should not exceed expected max height'
            )
            assert cropped.shape[1] <= 90, (
                f'{case_name} crop should not exceed expected max width'
            )
...
class TestRegionMatcher:
    ...
    def test_find_nearby_regions_with_visualization(
        self, matcher, test_regions_image, test_output_dir_regions
    ):
        ...
        cv2.imwrite(
            str(test_output_dir_regions / 'nearby_regions_visualization.png'), vis_image
        )
    ...
    def test_match_regions_with_detailed_output(
        self, matcher, test_regions_image, test_output_dir_regions
    ):
        ...
        matches = matcher.match_regions_by_proximity(
            targets, candidates, max_distance=200
        )
        ...
        cv2.imwrite(
            str(test_output_dir_regions / 'region_matches_visualization.png'), vis_image
        )
        ...
        with open(stats_file, 'w') as f:
            f.write('Region Matching Statistics\n')
            f.write('========================\n')

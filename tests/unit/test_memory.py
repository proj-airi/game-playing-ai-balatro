...
class TestMessage:
    ...
    def test_token_estimation(self):
        ...
        long_msg = Message(MessageRole.USER, "This is a longer message with more tokens")
...
class TestConversationState:
    ...
    def test_token_limit_management(self):
        ...
        state.add_user_message("This is a long message that should exceed the token limit")
    ...
    def test_clear_history(self):
        ...
        system_messages = [m for m in state.messages if m.role == MessageRole.SYSTEM]
        non_system_messages = [m for m in state.messages if m.role != MessageRole.SYSTEM]

        assert len(system_messages) >= 0  # May or may not have system message in messages
